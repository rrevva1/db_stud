-- Схема «авиакомпания» для лабораторной p2-l13 и практики части 3
-- Предметная область: рейсы, бронирования, пассажиры
...
-- Пример запроса: список бронирований с маршрутом
-- SELECT p.last_name, f.flight_no, a1.city AS from_city, a2.city AS to_city, b.seat_no
-- FROM booking b
-- JOIN passenger p ON p.passenger_id = b.passenger_id
-- JOIN flight f ON f.flight_id = b.flight_id
-- JOIN airport a1 ON a1.airport_code = f.dep_airport
-- JOIN airport a2 ON a2.airport_code = f.arr_airport;
